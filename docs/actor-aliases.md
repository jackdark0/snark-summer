...
| MrowLive | active participant / source channel | Mrow, Mraw, mrowlive, Morale | Use `MrowLive` for the channel; use `Mrow` only when the source clearly means the speaker. | Existing source labels use both forms. **`Morale` is a recurring whisper/caption mangle of Mrow** (user-confirmed 2026-07-04) — appears in EX-0030 and the "Fall of Counterpoints" transcript. Keep `Morale` inside direct quotes / source wording; use `Mrow` in authored prose. |
...
| Counterpoints | active participant | Conor, Connor, Conor Counterpoints | Use `Counterpoints` for the channel/person; `Conor` in short prose after first reference. | Former long-time Destiny friend ("six years"), since turned/aligned with the snark side — "lost to the snarkers" (user, 2026-06-16). Tactic actor in EX-0062/0067/0078 (I697f4pNk48, 2026-05-26). Earlier EX-0038–0040 ("Destiny vs Counterpoints") predate the turn; treat his role there as host/then-neutral unless the source shows otherwise. |
